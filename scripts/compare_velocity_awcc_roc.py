...
def main():
    ...
    reader.close()
    # Create a H5-file reader
    reader = H5Reader(path / 'reconstructed_with_roc.h5')
    # Read the cont. velocity
    u_p_rec_roc = reader.getDataSet('bubbles/velocity')[:]
    reader.close()
    # Create a H5-file reader
    reader = H5Reader(path / 'reconstructed_no_roc.h5')
    # Read the cont. velocity
    ...
    for jj,var_name in enumerate(var_names):
        ...
        (n_c, bins_c, patches_c) = axs.hist(np.clip(u_p_rec_roc[:,jj],limits[0],limits[1]), \
            bins=np.linspace(limits[0],limits[1],int((limits[1]-limits[0])/bin_width+1)), density=True, alpha=0.5, label='AWCC+ROC',\
            range=(limits[0],limits[1]))
        (n_c, bins_c, patches_c) = axs.hist(np.clip(u_p_rec[:,jj],limits[0],limits[1]), \
            bins=np.linspace(limits[0],limits[1],int((limits[1]-limits[0])/bin_width+1)), density=True, alpha=0.5, label='AWCC',\
            range=(limits[0],limits[1]))
        if jj == 0:
            axs.set_ylabel('Frequency [-]')
        axs.set_xlabel(f'Velocity $u_{var_name}$ [m/s]')
        axs.vlines(np.nanmedian(u_p_rec_roc[:,jj])+1.0537,0,max(n_c)*10, color='r', label='$\delta_{ROC}$')
        axs.vlines(np.nanmedian(u_p_rec_roc[:,jj])-1.0537,0,max(n_c)*10, color='r')
        axs.set_xlim(limits)
        axs.set_ylim([0,3.5])
        axs.text(limits[0]+0.1,3.2,'a)')
        axs.legend(loc=1,fontsize=7,ncol=1,frameon=False,fancybox=False,facecolor='w',edgecolor='k', bbox_to_anchor=(1.55,1.05))
        # if jj == 1:
        #     axs[jj].set_yscale('log')
        # if jj == 2:
        #     axs[jj].set_yscale('log')
    plt.subplots_adjust(left=0.15, bottom=0.2, right=0.7, top=0.95, wspace=0.3, hspace=None)
    fig.savefig(path / 'velocity_histrograms_awcc.svg',dpi=300)
    fig.savefig(path / 'velocity_histrograms_awcc.png',dpi=300)
