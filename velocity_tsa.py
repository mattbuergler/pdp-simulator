...
def main(args):
    ...
    for ii,k in enumerate(['x','y','z']):
        ...
        plt.ylim([min(np.nanmin(u_f[:,ii]),np.nanmin(u_f[:,ii])),max(np.nanmax(u_f[:,ii]),np.nanmax(u_f[:,ii]))])
    ...
    x_freq = np.logspace(2,3.2,1000)
    y_freq = np.power(x_freq,-5/3)/1
    ...
    plt.loglog(x_freq, y_freq, color='k', lw=0.75, linestyle='--' )
    plt.annotate('-5/3', (x_freq[round(len(x_freq)/2)],y_freq[round(len(y_freq)/2)]))
