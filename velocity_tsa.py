...
ts_frequency = int((len(t)-1)/total_duration)

# Accelrations for various sampling frequencies
fs1=7
dUdt_vel = np.diff(vel,axis=0)*ts_frequency
log10_first = 1
resampling_frequencies = np.flip(np.logspace(log10_first,math.log10(ts_frequency),int(math.log10(ts_frequency))))
fig, axs = plt.subplots(len(resampling_frequencies),len(directions),figsize=(len(directions)*2.5,len(resampling_frequencies)*2.5))
for ii,res_frq in enumerate(resampling_frequencies):
    n_timesteps = res_frq*total_duration
    res_indices = np.linspace(0,len(t)-1,int(n_timesteps)+1).astype('int')
    resampled_vel = vel[res_indices]
    dU = np.diff(resampled_vel,axis=0)
    dUdt = dU*res_frq
    for jj,di in enumerate(directions):
        (n, bins, patches) = axs[ii,jj].hist(dUdt[:,jj],bins=100,density=True, range=(min(dUdt_vel.reshape(-1)),max(dUdt_vel.reshape(-1))))
        axs[ii,jj].set_title(r'$\tau$ = {0} s'.format(1/res_frq))
        axs[ii,jj].text(0.99*min(dUdt_vel.reshape(-1)),0.95*max(n),r'$\mu$ = {:.1f}m/s$^{{-2}}$'.format(np.mean(dUdt[:,jj])),fontsize=fs1)
        axs[ii,jj].text(0.99*min(dUdt_vel.reshape(-1)),0.85*max(n),r'$\sigma$ = {:.0f}m/s$^{{-2}}$'.format(np.std(dUdt[:,jj])),fontsize=fs1)
...
log10_first = 1
resampling_frequencies = np.flip(np.logspace(log10_first,math.log10(ts_frequency),int(math.log10(ts_frequency))))
fig, axs = plt.subplots(ncols=len(resampling_frequencies),figsize=(len(resampling_frequencies)*2.5,2.5))
for jj,res_frq in enumerate(resampling_frequencies):
    ...
    axs[jj].text(0.99*min(dUdt_vel.reshape(-1)),0.95*max(n),r'$\mu$ = {:.1f}m/s$^{{-2}}$'.format(np.mean(dUdt[:,0])),fontsize=fs1)
    axs[jj].text(0.99*min(dUdt_vel.reshape(-1)),0.85*max(n),r'$\sigma$ = {:.0f}m/s$^{{-2}}$'.format(np.std(dUdt[:,0])),fontsize=fs1)
    if jj == 0:
...
dU = np.diff(vel,axis=0)
dUdt = dU*ts_frequency
(n, bins, patches) = plt.hist(dUdt[:,0],bins=100 ,density=True, \
        range=(min(dUdt_vel.reshape(-1)),max(dUdt_vel.reshape(-1))))
plt.title(r'$\delta t$ = {0} s'.format(1/ts_frequency))
plt.text(0.99*min(dUdt_vel.reshape(-1)),0.95*max(n),r'$\mu$ = {:.1f}m/s$^{{-2}}$'.format(np.mean(dUdt[:,0])),fontsize=fs1)
plt.text(0.99*min(dUdt_vel.reshape(-1)),0.85*max(n),r'$\sigma$ = {:.0f}m/s$^{{-2}}$'.format(np.std(dUdt[:,0])),fontsize=fs1)
...
plt.plot(t,vel[:,0],color=color,lw=lw,label='simulated, Langevin eq.')
plt.ylabel('$u_x$ [m/s]')
plt.xlabel('time $t$ [s]')
plt.xlim([0,flow_props['duration']])
plt.ylim([U_m[0]-5*U_m[0]*T_I[0],U_m[0]+5*U_m[0]*T_I[0]])
plt.title(r'$T_{{I,x}}$ = {}, $T_{{L,x}}$ = {}s, $dt$={}s, $T_{{L,x}}/dt$={}'.format(T_I[0],T_L[0],1.0/ts_frequency,int(T_L[0]*ts_frequency)),fontsize=fs2)
...
fig = plt.figure(figsize=(4.5,2.5))
plt.plot(t/T_L[0],(vel[:,0]-U_m[0])/(U_m[0]*T_I[0]),color='k', lw=0.75, \
    label='simulated, Langevin eq.')
plt.xlim([0,10])
plt.ylim([-4,4])
plt.xlabel('$t/T_L$ [-]')
...
acf,ci = acf(vel[:,0],nlags=lag,alpha=0.05,fft=False)
fig = plt.figure(figsize=(4.5,2.5))
plt.plot(np.linspace(0,lag,lag+1)/ts_frequency/T_L[0],acf,color='k',linestyle='-',label='simulated, Langevin eq.')
# plt.plot(np.linspace(0,lag,lag+1),ci,color='k',linestyle=':')
plt.plot(np.linspace(0,lag,lag+1)/ts_frequency/T_L[0],np.exp(-np.linspace(0,lag,lag+1)/ts_frequency/T_L[0]),color='k',linestyle=':',label='theoritical')
...
freqs, psd = signal.welch(vel[:,0], fs=ts_frequency)
fig = plt.figure(figsize=(4.5,2.5))
