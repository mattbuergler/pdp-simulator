...
plt.plot(t_f,u_f[:,0],color=color,lw=lw,label='fluid, Langevin eq.')
plt.plot(t_p,u_p[:,0],color='r',lw=lw,label='particle, Momentum eq.')
plt.ylabel('$u_x$ [m/s]')
plt.xlabel('time $t$ [s]')
plt.xlim([0,flow_props['duration']])
plt.ylim([U_m[0]-5*U_m[0]*T_I[0],U_m[0]+5*U_m[0]*T_I[0]])
plt.title(r'$T_{{I,x}}$ = {}, $T_{{L,x}}$ = {}s, \
            $dt$={}s, $T_{{L,x}}/dt$={}'.format( \
                    T_I[0], \
                    T_L[0], \
...
plt.plot(np.linspace(0,lag,lag+1)/ts_frequency/T_L[0], \
    acf,color='k',linestyle='-',label='simulated, Langevin eq.')
...
fig = plt.figure(figsize=(4.5,2.5))
plt.loglog(freqs_f, psd_f, color='k', lw=0.75, label='fluid, Langevin eq.' )
plt.loglog(freqs_p, psd_p, color='r', lw=0.75, label='particle, Momentum eq.' )
